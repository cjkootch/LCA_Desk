...
async function run() {
  ...
  const tables = [
    ...
    `CREATE TABLE IF NOT EXISTS announcements (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), title text NOT NULL, body text NOT NULL, priority text NOT NULL DEFAULT 'normal', target_roles text NOT NULL DEFAULT 'all', author_id uuid NOT NULL REFERENCES users(id), author_name text, status text NOT NULL DEFAULT 'draft', publish_at timestamp, expires_at timestamp, published_at timestamp, created_at timestamp DEFAULT now(), updated_at timestamp DEFAULT now())`,
  ];
