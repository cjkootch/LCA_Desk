...
export function UsageBanner() {
  const [data, setData] = useState<Awaited<
    ReturnType<typeof fetchPlanAndUsage>
  > | null>(null);
  ...
  if (plan.code === "enterprise") return null; // no limits to show
  ...
  if (plan.aiDraftsPerMonth > 0) {
    ...
    if (pct >= 60) {
      warnings.push({
        label: "AI Drafts",
        used: data.usage.aiDraftsUsed,
        limit: plan.aiDraftsPerMonth,
      });
    }
  }
  ...
  if (plan.aiChatMessagesPerMonth > 0) {
    const pct =
      (data.usage.aiChatMessagesUsed / plan.aiChatMessagesPerMonth) * 100;
    if (pct >= 60) {
      warnings.push({
        label: "AI Chat Messages",
        used: data.usage.aiChatMessagesUsed,
        limit: plan.aiChatMessagesPerMonth,
      });
    }
  }
  ...
  if (plan.entityLimit > 0 && data.usage.entityCount >= plan.entityLimit) {
    warnings.push({
      label: "Entities",
      used: data.usage.entityCount,
      limit: plan.entityLimit,
    });
  }
        ...
        <div className="flex items-start gap-3">
          <AlertTriangle
            className={cn(
              ...
              isAtLimit ? "text-warning" : "text-accent"
            )}
          />
          <div>
            ...
            <div className="mt-2 space-y-2">
              {warnings.map((w) => (
                <div key={w.label} className="text-sm">
                  ...
                  <Progress
                    value={Math.min((w.used / w.limit) * 100, 100)}
                    indicatorClassName={
                      w.used >= w.limit ? "bg-warning" : "bg-accent"
                    }
                  />
                </div>
              ))}
