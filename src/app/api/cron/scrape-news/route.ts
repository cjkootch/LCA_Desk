...
export async function GET(req: NextRequest) {
  ...
  try {
    console.log("News scrape triggered at", new Date().toISOString());
    // TODO: implement news scrape logic
    return NextResponse.json({
      ok: true,
      message: "News scrape triggered",
      timestamp: new Date().toISOString(),
    });
  } catch (err) {
    error = err instanceof Error ? err.message : String(err);
    console.error("scrape-news cron error:", err);
