...
function ModuleEditor({
  ...
  async function handleSave() {
    ...
    setSaving(true);
    try {
      await onSave({ title, content, passingScore });
    } finally {
      setSaving(false);
    }
  }
    ...
    <Card className="border-accent/30">
      <CardContent className="p-4 space-y-3">
        <div>
          ...
          <input className="input mt-1 w-full" value={title} onChange={e => setTitle(e.target.value)} placeholder="Module title" />
        </div>
        <div>
          <label className="text-xs font-medium text-text-muted">Content (Markdown)</label>
          <textarea
            className="input mt-1 w-full font-mono text-xs resize-y"
            rows={10}
            value={content}
...
export default function AdminCourseModulesPage() {
  ...
  async function load() {
    try {
      ...
      if (!course) { toast.error("Course not found"); router.replace("/dashboard/admin/courses"); return; }
  ...
  async function handleAdd(data: { title: string; content: string; passingScore: number }) {
    try {
      const mod = await addModule(courseId, { title: data.title, content: data.content, passingScore: data.passingScore });
  ...
  async function handleUpdate(moduleId: string, data: { title: string; content: string; passingScore: number }) {
    try {
      const updated = await updateModule(moduleId, { title: data.title, content: data.content, passingScore: data.passingScore });
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8 space-y-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-heading font-bold text-text-primary">{courseTitle}</h1>
          ...
          {modules.map(mod => (
            <div key={mod.id}>
              {editingId === mod.id ? (
                <ModuleEditor
                  mod={mod}
                  onSave={data => handleUpdate(mod.id, data)}
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-text-muted mt-0.5">
                          {mod.content ? `${mod.content.length} chars` : "No content"} · Pass: {mod.passingScore ?? 80}%
                        </p>
                      </div>
