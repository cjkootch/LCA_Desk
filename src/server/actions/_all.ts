...
import { eq, and, gte, lte, or, sql, desc, asc, isNull, ne, type InferInsertModel } from "drizzle-orm";
  ...
  return [
    {
      ...
      quiz: [
        ...
        { question: "What is the maximum fine for a first LCA offense?", options: ["$500,000", "$1,000,000", "$10,000,000", "$50,000,000"], correctIndex: 2 },
...
export async function seedLcaCourse() {
  ...
  const moduleData = _getLcaModuleData();
...
export async function fetchPlgStats() {
  ...
  const [
    ...
    // 2. Activation funnel — count distinct userId per eventName
    db.select({
    ...
    // 5. Latest run per job (get last 200 runs, dedup in JS)
    db.select({
      id: cronRuns.id,
  ...
  // Cron health — latest run per job
  ...
  // Trial list sorted by daysRemaining asc
...
async function checkSecretariatMember(userId: string) {
